...
if ! command -v jq &> /dev/null; then
    ...
    # Cập nhật và cài đặt jq (cần quyền sudo)
...
# 2. Upload ảnh vào Database (Ghi đè background-dark)
# assetName="background-dark" là từ khóa chính xác trong DB cho Dark Mode
...
SOURCE_FILES_DIR="./files" # Thư mục chứa các file code mẫu cần ghi đè
...
# 1. TẠO FILE CẤU HÌNH (Ghi đè hoặc Tạo mới)
echo "[1/5] Đang tạo file cấu hình brandingConfig.ts..."
...
echo "[2/5] Đang xử lý Logo và Assets..."
...
echo "[3/5] Đang ghi đè các file giao diện từ thư mục ./files ..."

if [ -d "$SOURCE_FILES_DIR" ]; then
    # 3.1 Ghi đè Sidebar Footer
    echo " -> Copy SidebarFooterWatermark.tsx..."
    cp -f "$SOURCE_FILES_DIR/SidebarFooterWatermark.tsx" "$METEOR_DIR/client/sidebar/footer/SidebarFooterWatermark.tsx"

    # 3.2 Ghi đè Login Powered By
    ...
    if [ -f "$DEST_LOGIN" ]; then
         cp -f "$SOURCE_FILES_DIR/LoginPoweredBy.tsx" "$DEST_LOGIN"
    else
         echo " -> Không tìm thấy đích đến cho LoginPoweredBy.tsx, đang tìm kiếm..."
         find packages -name "LoginPoweredBy.tsx" -exec cp "$SOURCE_FILES_DIR/LoginPoweredBy.tsx" {} \;
    fi
    ...
    # 3.3 Ghi đè Manifest ở public/images
...
echo "[4/5] Đang cấu hình giao diện theo màu chủ đạo..."

# CSS_FILE="$METEOR_DIR/app/theme/client/main.css"
# CUSTOM_CSS="
# /* --- CUSTOM BRANDING START --- */

# /*KHU VỰC NỘI DUNG CHÍNH (MAIN CONTENT & BUTTONS) */
# .rcx-content--main {
...
# # --- GHI VÀO FILE ---
# if [ -f "$CSS_FILE" ]; then
#     # Kiểm tra xem đã từng ghi CSS custom chưa để tránh ghi lặp lại
#     if grep -q "CUSTOM BRANDING START" "$CSS_FILE"; then
#         echo "CSS branding đã tồn tại trong file. Hãy xóa đoạn cũ trong main.css nếu muốn cập nhật lại."
#     else
#         echo "$CUSTOM_CSS" >> "$CSS_FILE"
#         echo "Đã áp dụng màu sắc (Sidebar: $COLOR_SIDEBAR_BG | Footer: Xám)."
#     fi
# else
#     echo "Lỗi: Không tìm thấy file $CSS_FILE"
# fi


# Định nghĩa nội dung CSS
...
# Sử dụng jq để tạo payload JSON an toàn (xử lý xuống dòng, ký tự đặc biệt)
# Setting ID cho Custom CSS trong Rocket.Chat là: Theme_custom_public_css
...
# 5. CẬP NHẬT BIẾN TRONG MANIFEST
echo "[5/5] Đang cập nhật nội dung manifest.json..."
